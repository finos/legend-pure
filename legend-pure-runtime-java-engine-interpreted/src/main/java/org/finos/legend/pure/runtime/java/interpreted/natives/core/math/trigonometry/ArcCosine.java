...
public class ArcCosine extends NativeFunction
{
    ...
    public ArcCosine(ModelRepository repository)
    {
    ...
    {
        Number input = NumericUtilities.toJavaNumber(Instance.getValueForMetaPropertyToOneResolved(params.get(0), M3Properties.values, processorSupport));
        double result = Math.acos(input.doubleValue());
        if (Double.isNaN(result))
        {
            throw new PureExecutionException(functionExpressionToUseInStack.getSourceInformation(), "Unable to compute acos of " + input.toString());
        }
