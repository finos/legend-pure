...
public class GetAll extends AbstractNative
{
    ...
    public String build(CoreInstance topLevelElement, CoreInstance functionExpression, ListIterable<String> transformedParams, ProcessorContext processorContext)
    {
        ...
        String name = null;
        if (processorSupport.instance_instanceOf(parametersValues.get(0), M3Paths.VariableExpression))
        {
            name = ValueSpecificationProcessor.processValueSpecification(topLevelElement, parametersValues.get(0), processorContext);
        }
        else
        {
            name = MetadataJavaPaths.buildMetadataKeyFromType(Instance.getValueForMetaPropertyToOneResolved(parametersValues.get(0), M3Properties.values, processorSupport));
        }
