...
public class ParallelMap extends NativeFunction
{
    private final FunctionExecutionInterpreted functionExecution;

    public ParallelMap(FunctionExecutionInterpreted functionExecution)
    {
    ...
    {
        ExecutorService executorService = functionExecution.getExecutorService();
        if (executorService == null)
        {
            Map map = new Map(functionExecution);
            return map.execute(params, resolvedTypeParameters, resolvedMultiplicityParameters, variableContext, functionExpressionToUseInStack, profiler, instantiationContext, executionSupport, context, processorSupport);
        }
        ...
        int parallelism = PrimitiveUtilities.getIntegerValue(Instance.getValueForMetaPropertyToOneResolved(params.get(2), M3Properties.values, processorSupport)).intValue();

        ListIterable<CoreInstance> results = ParallelHelper.flatCollect(collection, instance ->
        {
            ...
            return Lists.mutable.withAll(Instance.getValueForMetaPropertyToManyResolved(subResult, M3Properties.values, processorSupport));
        }, executorService, parallelism, executionSupport);
