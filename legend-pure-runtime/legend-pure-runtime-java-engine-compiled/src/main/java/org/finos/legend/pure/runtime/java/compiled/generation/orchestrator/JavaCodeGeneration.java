...
public class JavaCodeGeneration
{
    ...
    public static void doIt(GenerateParams generateParams)
    {
        ...
        genLogger.debug("  Requested repositories: " + generateParams.getRepositories());
        genLogger.debug("  Excluded repositories: " + generateParams.getExcludedRepositories());
        genLogger.debug("  Extra repositories: " + generateParams.getExtraRepositories());
        genLogger.debug("  Generation type: " + generateParams.getGenerationType());
        genLogger.debug("  Generate External API: addExternalAPI='" + generateParams.isAddExternalAPI() + "', in package '" + generateParams.getExternalAPIPackage() + "'");
        genLogger.debug("  classesDirectory: '" + generateParams.getTargetDirectoryForResources());
        genLogger.debug("  targetDirectoryForSources: '" + generateParams.getTargetDirectoryForSources());
        genLogger.debug("  targetDirectoryForMetadata: '" + generateParams.getTargetDirectoryForMetadata());
        genLogger.debug("  Other options: " +
        ...
        genLogger.debug("  Working directory: '" + FileSystems.getDefault().getPath("").toAbsolutePath());

        try
        {
            CodeRepositorySet allRepositories = getAllRepositories(generateParams.getExtraRepositories());
            genLogger.debug(allRepositories.getRepositories().collect(CodeRepository::getName).makeString("  Found repositories: ", ", ", ""));
            genLogger.debug(new ParserService().parsers().collect(Parser::getName).makeString("  Found parsers: ", ", ", ""));
            genLogger.debug(new ParserService().inlineDSLs().collect(InlineDSL::getName).makeString("  Found DSL parsers: ", ", ", ""));
            ...
            genLogger.debug(selectedRepositories.makeString("  Selected repositories: ", ", ", ""));
            ...
            else
            {
                ...
                Generate generate = generate(
                        ...
                        generateParams.isGeneratePureTests(),
                        genLogger
                    );
    ...
    public static class GenerateParams
    {
        ...
        public boolean isGeneratePureTests()
        {
            return this.generatePureTests;
        }
    }
}
