...
public final class ResultSetValueHandlers
{
    ...
    private static final ResultSetValueHandler DATE = (rs, i, nullSqlInstance, calendar) ->
    {
        Date date = rs.getDate(i);
        return date == null ? nullSqlInstance : StrictDate.fromSQLDate(date);
    };

    private static final ResultSetValueHandler TIMESTAMP = (rs, i, nullSqlInstance, calendar) ->
    {
    ...
    private static final ImmutableIntObjectMap<ResultSetValueHandler> HANDLERS = IntObjectMaps.mutable.<ResultSetValueHandler>empty()
            ...
            .withKeyValue(Types.DATE, DATE)
            .withKeyValue(Types.TIMESTAMP, TIMESTAMP)
            .withKeyValue(Types.TIMESTAMP_WITH_TIMEZONE, TIMESTAMP)
    ...
    public static ListIterable<ResultSetValueHandler> getHandlers(ResultSetMetaData metaData) throws SQLException
    {
        ...
        for (int i = 1; i <= count; i++)
        {
            ...
            ResultSetValueHandler handler = DB_SPECIFIC_HANDLERS.getIfAbsent(columnTypeName, () -> HANDLERS.get(columnType));
            if (handler == null)
            {
