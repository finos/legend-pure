...
public class FSPeltGraphLoaderHybridPureGraphCache extends AbstractFSDirectoryPureGraphCache
{
    ...
    public FSPeltGraphLoaderHybridPureGraphCache(Path cacheDirectory, boolean allowBuildingFromRepoSubset, ForkJoinPool forkJoinPool, Message message)
    {
    ...
    public FSPeltGraphLoaderHybridPureGraphCache(Path cacheDirectory, boolean allowBuildingFromRepoSubset, Message message)
    {
        this(cacheDirectory, allowBuildingFromRepoSubset, null, message);
    }

    public FSPeltGraphLoaderHybridPureGraphCache(Path cacheDirectory, Message message)
    {
        this(cacheDirectory, false, null, message);
    }

    public FSPeltGraphLoaderHybridPureGraphCache(Path cacheDirectory)
    {
    ...
    public boolean buildFromCaches(ModelRepository modelRepository, SourceRegistry sources, ParserLibrary library, Context context, ProcessorSupport processorSupport, Message message)
    {
        ...
        PureCompilerLoader peltLoader = PureCompilerLoader.newLoader(Thread.currentThread().getContextClassLoader(), getCacheLocation());
        MutableList<String> peltRepos = repoNames.select(peltLoader::canLoad);
        if (peltRepos.size() == repoNames.size())
        {
            peltLoader.load(this.pureRuntime, peltRepos);
            updateCacheState();
            return true;
        }

        PureRepositoryJarLibrary jarLibrary = SimplePureRepositoryJarLibrary.newLibraryFromDirectory(getCacheLocation());
        MutableList<String> parRepos = repoNames.select(jarLibrary::isKnownRepository);
        if (parRepos.size() == repoNames.size())
        {
            GraphLoader graphLoader = new GraphLoader(modelRepository, context, library, this.pureRuntime.getIncrementalCompiler().getDslLibrary(), sources, null, jarLibrary, this.forkJoinPool);
            parRepos.sortThis(new RepositoryComparator(codeStorage.getAllRepositories()))
                    .forEach(repoName -> graphLoader.loadRepository(repoName, message));
            updateCacheState();
            return true;
        }
        ...
        {
            if (peltRepos.size() >= parRepos.size())
            {
                peltLoader.load(this.pureRuntime, peltRepos);
            }
            else
        ...
        {
            // try to load everything (which will probably fail)
            peltLoader.load(this.pureRuntime, repoNames);
        }
        updateCacheState();
        return true;
    }
