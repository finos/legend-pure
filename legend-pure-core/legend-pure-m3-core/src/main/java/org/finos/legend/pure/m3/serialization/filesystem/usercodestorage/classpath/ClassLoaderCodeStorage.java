...
public class ClassLoaderCodeStorage extends AbstractMultipleRepositoryCodeStorage implements ImmutableRepositoryCodeStorage
{
    ...
    private ImmutableMap<String, ImmutableMap<String, ClassLoaderCodeStorageNode>> buildNodeMap()
    {
        MutableSet<String> repositoryNames = getAllRepositories().collect(CodeRepository::getName, Sets.mutable.empty());
        ...
        if (repositoryNames.notEmpty())
        {
            ...
            findURLsByPath(repositoryNames).forEachKeyValue((path, urls) ->
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        urls.injectInto((byte[]) null, (bytes, url) ->
                        {
                            byte[] content = readURLContent(url);
                            if ((bytes != null) && !Arrays.equals(bytes, content))
                            {
            ...
            // Any remaining repositories are empty: add an empty directory for each
            repositoryNames.forEach(repositoryName ->
            {
                String path = CodeStorageTools.canonicalizePath(repositoryName);
                result.put(repositoryName, Maps.immutable.with(path, new ClassLoaderDirectoryNode(path)));
            });
        }
    ...
    private MutableMap<String, MutableSet<URL>> findURLsByPath(Iterable<String> repositoryNames)
    {
        MutableMap<String, MutableSet<URL>> pathURLs = Maps.mutable.empty();
        try
        {
    ...
    private void collectFromPath(String parentPath, Path filePath, MutableMap<String, MutableSet<URL>> pathURLs)
    {
        String path = CodeStorageTools.joinPaths(parentPath, filePath.getFileName().toString());
        pathURLs.getIfAbsentPut(path, Sets.mutable::empty).add(pathToURL(filePath.toAbsolutePath().normalize()));
    ...
    private void collectFromJarFile(JarFile jarFile, String jarFileURL, MutableMap<String, MutableSet<URL>> pathURLs)
    {
        ...
        while (entries.hasMoreElements())
        {
            ...
            {
                pathURLs.getIfAbsentPut(CodeStorageTools.canonicalizePath(entryName), Sets.mutable::empty).add(createJarEntryURL(jarFileURL, entryName));
            }
        }
    }

    private URL createJarEntryURL(String jarFileURL, String entryName)
    {
        try
        {
    ...
    private static byte[] readURLContent(URL url)
    {
