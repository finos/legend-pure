...
public class CompiledStateIntegrityTestTools
{
    ...
    static <T extends CoreInstance> MutableSet<T> forEachInstancePath(Iterable<T> instances, Iterable<? extends CoreInstance> startNodes, ProcessorSupport processorSupport, BiConsumer<? super T, ? super ResolvedGraphPath> consumer)
    {
        ...
        for (CoreInstance startNode : (startNodes == null) ? GraphTools.getTopLevelAndPackagedElements(processorSupport).select(n -> n.getSourceInformation() != null) : startNodes)
        {
            for (ResolvedGraphPath rgp : GraphTools.internalGraphPaths(startNode, processorSupport))
            {
    ...
    private static void runIntegrityTest(Iterable<? extends CoreInstance> instances, String violationDescription, BiConsumer<CoreInstance, Consumer<? super String>> test, boolean filterInstancesFromDependencyGraph, String baseRepository)
    {
        ...
        if (filterInstancesFromDependencyGraph && baseRepository != null)
        {
            instances.forEach(i ->
            {
                if (i.getSourceInformation() != null && i.getSourceInformation().getSourceId() != null && i.getSourceInformation().getSourceId().startsWith("/" + baseRepository))
                {
                    test.accept(i, errorMessages::add);
                }
            });
        }
        else
...
//                            violationConsumer.accept(message.toString());
                    ...
                    else if (!GenericType.isGenericCompatibleWith(valueGenericType, genericType, processorSupport))
                    {
                        ...
                        ResolvedGraphPath path = GraphTools.findPathToInstance(instance, processorSupport);
                        if (path != null)
                        {
                            if (sourceInfo == null)
                            {
