...
public class MilestoningFunctionExpressionValidator
{
    ...
    {
        validateLatestDateUsage(functionExpression, function, repository, processorSupport);
        validateMissingMilestoningDateArguments(functionExpression, function, processorSupport);
    }

    private static void validateLatestDateUsage(FunctionExpression functionExpression, Function function, ModelRepository repository, ProcessorSupport processorSupport)
    {
        RichIterable<CoreInstance> funcParams = functionExpression._parametersValues().flatCollect(MilestoningFunctions.toInstanceValues(processorSupport));
        boolean hasLatestDateParam = funcParams.anySatisfy(MilestoningFunctions.isLatestDate(processorSupport));
        boolean isGetAllMilestoningFunction = FastList.newListWith(MilestoningFunctions.MILESTONING_GET_ALL_FUNCTION_PATH, MilestoningFunctions.BITEMPORAL_MILESTONING_GET_ALL_FUNCTION_PATH).anySatisfy(Predicates.equal(PackageableElement.getUserPathForPackageableElement(function)));
