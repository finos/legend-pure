...
public class GroupBy extends AbstractNative implements Native
{
    ...
    public String build(CoreInstance topLevelElement, CoreInstance functionExpression, ListIterable<String> transformedParams, ProcessorContext processorContext)
    {
        ...
        result.append(transformedParams.get(2));
        result.append("._map(),es), ");
        result.append("(Function2)PureCompiledLambda.getPureFunction(");
        result.append(transformedParams.get(2));
        result.append("._reduce(),es), ");
        result.append("((org.finos.legend.pure.m3.coreinstance.meta.pure.metamodel.type.FunctionType)");
        result.append(transformedParams.get(2));
        result.append("._reduce()._classifierGenericType()._typeArguments().toList().get(0)._rawType())._returnType()._rawType()._name(), es)\n");
