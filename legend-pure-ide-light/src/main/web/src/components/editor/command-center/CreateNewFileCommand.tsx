...
export const CreateNewFileCommand = observer(() => {
  ...
  // actions
  const closeModal = (): void => editorStore.directoryTreeState.setNodeForCreateNewFile(undefined);
  ...
  const create = (event: React.FormEvent<HTMLFormElement | HTMLButtonElement>): void => {
    ...
    closeModal();
    flowResult(editorStore.createNewFile(`${currentNode.data.li_attr.path}/${value}`)).catch(applicationStore.alertIllegalUnhandledError);
  };
